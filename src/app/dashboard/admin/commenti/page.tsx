...
export default function AdminCommentiPage() {
  ...
  async function load() {
    ...
    try {
      ...
      const res = await fetch('/api/admin/comments', { headers: token ? { 'Authorization': `Bearer ${token}` } : undefined });
  ...
  async function act(id: number, action: 'APPROVE' | 'REJECT' | 'DELETE') {
    ...
    try {
      ...
      const res = await fetch('/api/admin/comments', {
        method: action === 'DELETE' ? 'PATCH' : 'PATCH',
        headers: { 'Content-Type': 'application/json', ...(token ? { 'Authorization': `Bearer ${token}` } : {}) },
        body: JSON.stringify({ id, action: action === 'DELETE' ? 'delete' : action.toLowerCase() })
      });
  ...
  return (
    <div className="space-y-6">
      <SectionHeader title="Admin · Moderazione Commenti" subtitle="Approva o rifiuta i commenti inviati dagli utenti" />

      <div className="rounded-xl border border-gray-600 bg-gray-800 p-5">
        {loading ? (
          <div className="text-sm text-gray-400">Caricamento…</div>
        ) : list.length === 0 ? (
          <div className="text-sm text-gray-400">Nessun commento in moderazione</div>
        ) : (
              ...
              <div key={r.id} className="border border-gray-600 bg-gray-900 rounded-md p-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="space-y-1">
                    ...
                    <div className="text-xs text-gray-500">Inviato: {new Date(r.createdAt).toLocaleString()}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <Button variant="destructive" disabled={acting===r.id} onClick={()=>act(r.id, 'DELETE')}>{acting===r.id ? '...' : 'Elimina'}</Button>
                  </div>
                </div>
              </div>
            ))}
