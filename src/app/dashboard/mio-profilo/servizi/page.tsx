    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-gray-600 bg-gray-800 p-5 space-y-6">
        {SERVICE_GROUPS.map((g) => (
          <div key={g.name} className="border border-gray-700 rounded-lg">
            <button
              type="button"
              onClick={() => setOpen((p) => ({ ...p, [g.name]: !p[g.name] }))}
              className="w-full flex items-center justify-between px-3 py-2 bg-gray-900 rounded-t-lg"
            >
              <div className="font-semibold text-white">{g.name}</div>
              <div className="flex items-center gap-3 text-xs text-gray-400">
                <span>{g.items.filter((i) => state?.[i.key]?.enabled).length}/{g.items.length}</span>
                <span className={`transition-transform ${open[g.name] ? 'rotate-180' : ''}`}>⌄</span>
              </div>
            </button>
            {open[g.name] && (
