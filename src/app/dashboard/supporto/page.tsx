...
export default function SupportoPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border bg-white p-4">
        <h3 className="font-semibold mb-3">Apri un Ticket</h3>
        <form onSubmit={send} className="grid gap-3 max-w-xl">
          <input value={subject} onChange={(e) => setSubject(e.target.value)} placeholder="Oggetto" className="border rounded-md px-3 py-2" required />
          <textarea value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Descrivi il problema" className="border rounded-md px-3 py-2 h-28" required />
