        ...
        {loading ? (
          ...
          escortsFiltrate.map((escort:any) => (
            <Link key={escort.id} href={`/escort/${escort.slug || `${kebab(escort.name)}-${escort.id}`}`}>
              <EscortCard escort={{ id: escort.id, nome: escort.name, eta: Number(escort.eta) || 0, citta: Array.isArray(escort.cities)&&escort.cities[0]?String(escort.cities[0]):'—', capelli: '', prezzo: escort.price || 0, foto: escort.coverUrl || "/placeholder.svg", rank: escort.tier, isVerified: !!escort.hasApprovedDoc, videoCount: escort.videoCount || 0, reviewCount: escort.reviewCount || 0, commentCount: escort.commentCount || 0 }} />
            </Link>
          ))
        )}
      </div>
