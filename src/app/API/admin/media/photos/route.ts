...
export async function GET(request: NextRequest) {
  try {
    ...
    const items = await prisma.photo.findMany({ where: { status: status as any }, orderBy: { createdAt: 'desc' }, take: 100 });
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const item = await prisma.photo.update({ where: { id }, data: { status: status as any } });
