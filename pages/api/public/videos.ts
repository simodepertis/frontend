...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const perPage = Math.min(48, Math.max(1, Number((req.query.perPage as string) || 12)))
    const city = String((req.query.city as string) || '').trim().toLowerCase()
    const tag = String((req.query.tag as string) || '').trim().toLowerCase()
    ...
    const itemsBase = videos.map((v) => {
      ...
      return {
        ...
        thumb: normalizeUrl(v.thumb) || '/placeholder.png',
    ...
    console.error('❌ API pages/public/videos error:', e)
