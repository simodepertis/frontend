...
export default async function handler(
  ...
  console.log('📦 API /api/credits/catalog chiamata (Pages Router)')
  ...
  try {
    ...
    console.log(`✅ Trovati ${list.length} pacchetti crediti`)
    return res.status(200).json({ products: list });
  } catch (error: unknown) {
    console.error('❌ ERRORE in /api/credits/catalog:', error)
