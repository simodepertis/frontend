...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'GET') {
    try {
      ...
      const items = await prisma.comment.findMany({
        ...
        take: 100,
        select: { id: true, body: true, createdAt: true, author: { select: { id: true, nome: true, email: true } }, target: { select: { id: true, nome: true, slug: true } } }
      });
  ...
  if (req.method === 'PATCH') {
    try {
      ...
      if (!idNum || !['approve','reject','delete'].includes(act)) return res.status(400).json({ error: 'Parametri non validi' });
      if (act === 'delete') {
        await prisma.comment.delete({ where: { id: idNum } });
        return res.status(200).json({ ok: true });
      }
      const status = act === 'approve' ? 'APPROVED' : 'REJECTED';
      const item = await prisma.comment.update({ where: { id: idNum }, data: { status: status as any } });
